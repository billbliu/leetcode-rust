...
impl Solution {
    ...
    pub fn search_range_1(&self, nums: Vec<i32>, target: i32) -> Vec<i32> {
        ...
        // vec![left, right]
        vec![
            self.search_first_equal_element(&nums, target),
            self.search_last_equal_element(&nums, target),
        ]
    }
...
mod tests {
    ...
    fn test_34_1() {
        ...
        assert_eq!(
            vec![3, 4],
            solution.search_range_1(vec![5, 7, 7, 8, 8, 10], 8)
        );
        assert_eq!(
            vec![-1, -1],
            solution.search_range_1(vec![5, 7, 7, 8, 8, 10], 6)
        );
